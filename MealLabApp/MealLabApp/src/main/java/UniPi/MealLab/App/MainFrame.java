...
public class MainFrame extends JFrame {
    ...
    public void performSearch(String query, boolean byName) {
        ...
        if (byName) {
             runWorker(() -> service.searchRecipes(query), searchModel, searchBtn, progressBar);
        } else {
             runWorker(() -> service.getRecipesByIngredient(query), searchModel, searchBtn, progressBar);
        }
    }
    ...
    private JPanel createSplitView(DefaultListModel<Recipe> model, boolean isSearchTab) {
        ...
        if (isSearchTab) {
            ...
            searchField.setMinimumSize(new Dimension(200, 45));
            
            searchTypeCombo = new JComboBox<>(new String[]{"Name", "Ingredient"});
            searchTypeCombo.setFont(FONT_PLAIN);
            searchTypeCombo.setPreferredSize(new Dimension(100, 45));
            
            searchBtn = new ModernButton("Search", ACCENT, Color.WHITE);
            ...
            gbc.gridx = 1; 
            centerPanel.add(searchTypeCombo, gbc);
            ...
            botPanel.add(progressBar, BorderLayout.CENTER);
            top.add(botPanel, BorderLayout.SOUTH);
            
            // Search and Random button listeners
            ActionListener searchAction = e -> {
                String q = searchField.getText().trim();
                boolean byName = searchTypeCombo.getSelectedItem().equals("Name");
                performSearch(q, byName);
            };
    ...
    boolean addUnique(DefaultListModel<Recipe> m, Recipe r) { 
